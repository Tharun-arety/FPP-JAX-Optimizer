...
def test_zero_ply_patches_do_not_contribute_kinematic_penalty() -> None:
    dome = build_type_iv_dome(DomeConfig(theta_points=28, phi_points=48))
    material = MaterialConfig()
    config = OptimizationConfig(patch_count=1)
    layout = {
        ...
        "width_m": jnp.asarray([0.12], dtype=jnp.float32),
        "angle_rad": jnp.asarray([0.5 * jnp.pi - 0.03], dtype=jnp.float32),
        "plies": jnp.asarray([0.0], dtype=jnp.float32),
        "transition_theta": jnp.asarray(dome.config.theta_open + 0.2, dtype=jnp.float32),
    }

    kinematics = evaluate_kinematics(dome, layout, material, config)
    assert float(kinematics["penalty"]) == 0.0
    assert float(kinematics["max_shear"]) == 0.0


def test_reported_kinematic_peaks_match_supported_patch_region() -> None:
    dome = build_type_iv_dome(DomeConfig(theta_points=28, phi_points=48))
    material = MaterialConfig()
    config = OptimizationConfig(patch_count=1)
    layout = {
        ...
        "center_phi": jnp.asarray([0.0], dtype=jnp.float32),
        "length_m": jnp.asarray([0.12], dtype=jnp.float32),
        "width_m": jnp.asarray([0.10], dtype=jnp.float32),
        "angle_rad": jnp.asarray([0.25 * jnp.pi], dtype=jnp.float32),
        "plies": jnp.asarray([1.5], dtype=jnp.float32),
        "transition_theta": jnp.asarray(dome.config.theta_open + 0.24, dtype=jnp.float32),
    }

    kinematics = evaluate_kinematics(dome, layout, material, config)
    report_mask = np.asarray(kinematics["report_mask"])
    shear = np.asarray(kinematics["shear_per_patch"])
    areal = np.asarray(kinematics["areal_distortion_per_patch"])

    assert bool(report_mask.any())
    assert np.isclose(float(kinematics["max_shear"]), float(np.max(shear[report_mask])))
    assert np.isclose(float(kinematics["max_areal_distortion"]), float(np.max(areal[report_mask])))
